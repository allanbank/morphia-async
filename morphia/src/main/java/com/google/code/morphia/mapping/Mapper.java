 ...
 * 
 * <p>
 * This is the heart of Morphia and takes care of mapping from/to
 * POJOs/DBObjects
 * <p>
 * <p>
...
public class Mapper {
    ...
    /**
     * Special name that can never be used. Used as default for some fields to
     * indicate default state.
     */
    public static final String IGNORED_FIELDNAME = ".";
    /**
     * Special field used by morphia to support various possibly loading issues;
     * will be replaced when discriminators are implemented to support
     * polymorphism
     */
    ...
    // EntityInterceptors; these are called before EntityListeners and lifecycle
    // methods on an Entity, for all Entities
    private final List<EntityInterceptor> interceptors = new LinkedList<EntityInterceptor>();

    // A general cache of instances of classes; used by MappedClass for
    // EntityListerner(s)
    ...
    LazyProxyFactory proxyFactory = LazyFeatureDependencies
    ...
    private MappedClass addMappedClass(MappedClass mc, boolean validate) {
        ...
        Set<MappedClass> mcs = mappedClassesByCollection.get(mc
                .getCollectionName());
        if (mcs == null) {
            mcs = new CopyOnWriteArraySet<MappedClass>();
            Set<MappedClass> temp = mappedClassesByCollection.putIfAbsent(
                    mc.getCollectionName(), mcs);
            if (temp != null)
                mcs = temp;
        }
    ...
    public void updateKeyInfo(final Object entity, final Document dbObj,
        ...
        if ((mc.getIdField() != null) && (dbObj != null)
                && (dbObj.get(ID_KEY) != null)) {
            try {
                ...
                if (oldIdValue != null) {
                    ...
                    if (!dbIdValue.equals(oldIdValue)) {
                        mf.setFieldValue(entity, oldIdValue);// put the value
                                                             // back...

                        throw new RuntimeException("@Id mismatch: "
                                + oldIdValue + " != " + dbIdValue + " for "
                ...
                throw new RuntimeException(
                        "Error setting @Id field after save/insert.", e);
            }
        }
    }
     ...
     * 
     * @param entityClass
     *            The type to return, or use; can be overridden by the @see
     *            Mapper.CLASS_NAME_FIELDNAME in the DBObject
     **/
    public Object fromDBObject(final Class entityClass,
            final Document dbObject, EntityCache cache) {
        if (dbObject == null) {
    ...
    Object toMongoObject(Object javaObj, boolean includeClassName) {
        ...
        if (!bSameType
                && !(Map.class.isAssignableFrom(type) || Iterable.class
                        .isAssignableFrom(type)))
            return newObj;
        else { // The converter ran, and produced another type, or it is a
               // list/map
            ...
            if (isSingleValue && !ReflectionUtils.isPropertyType(type)) {
                ...
                if (ReflectionUtils.isPropertyType(subType))
                    ...
                    for (Map.Entry e : (Iterable<Map.Entry>) ((Map) newObj)
                            .entrySet())
                        m.put(e.getKey(),
                                toMongoObject(e.getValue(), includeClassName));
        ...
        (value instanceof Iterable && mf.isMultipleValues() && (mf
            ...
            try {
                if (value instanceof Iterable) {
                    ...
                    Key<?> k = (value instanceof Key) ? (Key<?>) value
                            : getKey(value);
                ...
                log.error(
                        "Error converting value(" + value + ") to reference.",
                        e);
            ...
            try {
                mappedValue = Serializer.serialize(value,
                        !mf.getAnnotation(Serialized.class)
                                .disableCompression());
            }
            catch (IOException e) {
    ...
    /**
     * <p>
     * Converts an entity (POJO) to a DBObject; A special field will be added to
     * keep track of the class: {@link Mapper.CLASS_NAME_FIELDNAME}
     * </p>
     * 
     * @param entity
     *            The POJO
    ...
    /**
     * <p>
     * Converts an entity (POJO) to a DBObject (for use with low-level driver);
     * A special field will be added to keep track of the class:
     ...
     */
    public Document toDBObject(Object entity,
            Map<Object, Document> involvedObjects) {
        return toDBObject(entity, involvedObjects, true);
    }

    Document toDBObject(Object entity, Map<Object, Document> involvedObjects,
            boolean lifecycle) {

        DocumentBuilder dbObject = BuilderFactory.start();
        MappedClass mc = getMappedClass(entity);

        if (mc.getEntityAnnotation() == null
                || !mc.getEntityAnnotation().noClassnameStored())
    ...
    private void writeMappedField(DocumentBuilder dbObject, MappedField mf,
        ...
        for (Class<? extends Annotation> testType : new Class[] {
                Property.class, Embedded.class, Serialized.class,
                Reference.class }) {
    ...
    public <T> Key<T> refToKey(DocumentReference ref) {
        if (ref == null)
            return null;
    ...
    public DocumentReference keyToRef(Key key) {
        if (key == null)
            return null;
    ...
    /**
     * Validate the path, and value type, returning the mappedfield for the
     * field at the path
     */
    public static MappedField validate(Class clazz, Mapper mapr,
            StringBuffer origProp, FilterOperator op, Object val,
            boolean validateNames, boolean validateTypes) {
        ...
        if (validateNames) {
            ...
            for (int i = 0;;) {
                ...
                // catch people trying to search/update into
                // @Reference/@Serialized fields
                if (i < parts.length && !canQueryPast(mf))
                    throw new ValidationException(
                            "Can not use dot-notation past '" + part
                                    + "' could not be found in '"
                ...
                mc = mapr.getMappedClass((mf.isSingleValue()) ? mf.getType()
                        : mf.getSubClass());
            }

            // record new property string if there has been a translation to any
            // part
            if (hasTranslations) {
            ...
            if (validateTypes)
                if ((mf.isSingleValue() && !isCompatibleForOperator(
                        mf.getType(), op, val))
                        || ((mf.isMultipleValues() && !(isCompatibleForOperator(
                                mf.getSubClass(), op, val) || isCompatibleForOperator(
                                mf.getType(), op, val))))) {
    ...
    private static boolean canQueryPast(MappedField mf) {
        return !(mf.hasAnnotation(Reference.class) || mf
                .hasAnnotation(Serialized.class));
    }

    public static boolean isCompatibleForOperator(Class<?> type,
        ...
        if (value == null || type == null)
            ...
            return true;
                ...
                && (value.getClass().isArray()
                        || Iterable.class.isAssignableFrom(value.getClass()) || Map.class
            ...
            return true;
                ...
                && (int.class.equals(type) || long.class.equals(type) || Long.class
                        .equals(type)))
            return true;
        ...
        // hack to let Long match long, and so on
                !value.getClass().getSimpleName().toLowerCase()
                        .equals(type.getSimpleName().toLowerCase())) {
            return false;
        }
        return true;
    }
    ...
    public Class<?> getClassFromKind(String kind) {
        ...
        if (mcs.isEmpty())
            throw new MappingException("The collection '" + kind
                    + "' is not mapped to a java class.");
        if (mcs.size() > 1)
            if (log.isInfoEnabled())
                log.info("Found more than one class mapped to collection '"
                        + kind + "'" + mcs);
