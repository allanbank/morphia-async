...
public class DatastoreImpl implements Datastore, AdvancedDatastore {
    ...
    protected Durability defConcern = Durability.ACK;

    public DatastoreImpl(Mapper mapr, MongoClient mongo, String dbName) {
        this.mapr = mapr;
        this.mongo = mongo;
    ...
    public DatastoreImpl(Morphia morphia, MongoClient mongo) {
    ...
    public <T, V> DocumentReference createRef(Class<T> clazz, V id) {
        if (id == null)
            throw new MappingException("Could not get id for "
                    + clazz.getName());
        return new DocumentReference(getDB().getName(), getCollection(clazz)
                .getName(), BuilderFactory.start().add("_id", id).build()
                .get("_id"));
    }
    ...
    public <T, V> long delete(Class<T> clazz, Iterable<V> ids) {
        Query<T> q = find(clazz).disableValidation().filter(
                Mapper.ID_KEY + " in", ids);
        return delete(q);
    }
    ...
    public <T> long delete(Query<T> query, WriteConcern wc) {
        ...
        if (q.getSortObject() != null || q.getOffset() != 0 || q.getLimit() > 0)
            throw new QueryException(
                    "Delete does not allow sort/offset/limit query options.");
    ...
    public <T> void ensureIndex(Class<T> clazz, String name, String fields,
        ...
        ensureIndex(clazz, name,
                QueryImpl.parseFieldsString(fields, clazz, mapr, true), unique,
                dropDupsOnCreate, false, false);
    }

    public <T> void ensureIndex(Class<T> clazz, String name, String fields,
            boolean unique, boolean dropDupsOnCreate, boolean background) {
        ensureIndex(clazz, name,
                QueryImpl.parseFieldsString(fields, clazz, mapr, true), unique,
                dropDupsOnCreate, background, false);
    }
    ...
    protected <T> void ensureIndex(Class<T> clazz, String name,
        ...
        MongoCollection dbColl = getCollection(clazz);

        BasicDBObject opts = (BasicDBObject) keyOpts.get();
        if (opts.isEmpty()) {
            log.debug("Ensuring index for " + dbColl.getName() + " with keys:"
                    + fields);
            dbColl.ensureIndex(fields);
        }
        else {
            log.debug("Ensuring index for " + dbColl.getName() + " with keys:"
                    + fields + " and opts:" + opts);
            dbColl.ensureIndex(fields, opts);
        }
    }

    @SuppressWarnings({ "rawtypes" })
    public void ensureIndex(Class clazz, String name, IndexFieldDef[] defs,
            boolean unique, boolean dropDupsOnCreate, boolean background) {
        ...
        ensureIndex(clazz, name, (BasicDBObject) keys.get(), unique,
                dropDupsOnCreate, background, false);
    }
    ...
    public <T> void ensureIndex(Class<T> type, boolean background,
    ...
    protected void ensureIndexes(MappedClass mc, boolean background) {
        ensureIndexes(mc, background, new ArrayList<MappedClass>(),
                new ArrayList<MappedField>());
    }

    protected void ensureIndexes(MappedClass mc, boolean background,
            ArrayList<MappedClass> parentMCs, ArrayList<MappedField> parentMFs) {
        if (parentMCs.contains(mc))
            return;

        // skip embedded types
        if (mc.getEmbeddedAnnotation() != null
                && (parentMCs == null || parentMCs.isEmpty()))
            return;
        ...
        for (MappedField mf : mc.getPersistenceFields()) {
            if (mf.hasAnnotation(Indexed.class)) {
                ...
                Class<?> indexedClass = (parentMCs.isEmpty() ? mc : parentMCs
                        .get(0)).getClazz();
                ...
                ensureIndex(indexedClass, index.name(),
                        new BasicDBObject(field.toString(), index.value()
                                .toIndexValue()), index.unique(),
                        index.dropDups(),
            ...
            if (!mf.isTypeMongoCompatible()
                ...
                ArrayList<MappedClass> newParentClasses = (ArrayList<MappedClass>) parentMCs
                        .clone();
                ArrayList<MappedField> newParents = (ArrayList<MappedField>) parentMFs
                        .clone();
                newParentClasses.add(mc);
                newParents.add(mf);
                ensureIndexes(mapr.getMappedClass(mf.isSingleValue() ? mf
                        .getType() : mf.getSubClass()), background,
    ...
    public void ensureCaps() {
        for (MappedClass mc : mapr.getMappedClasses())
            if (mc.getEntityAnnotation() != null
                    && mc.getEntityAnnotation().cap().value() > 0) {
                CappedAt cap = mc.getEntityAnnotation().cap();
                String collName = mapr.getCollectionName(mc.getClazz());
                BasicDBObjectBuilder dbCapOpts = BasicDBObjectBuilder.start(
                        "capped", true);
                if (cap.value() > 0)
                ...
                if (db.getCollectionNames().contains(collName)) {
                    DBObject dbResult = db.command(BasicDBObjectBuilder.start(
                            "collstats", collName).get());
                    if (dbResult.containsField("capped")) {
                        // TODO: check the cap options.
                        log.warning("MongoCollection already exists is cap'd already; doing nothing. "
                                + dbResult);
                    }
                    else {
                        log.warning("MongoCollection already exists with same name("
                                + collName
                    ...
                    log.debug("Created cap'd MongoCollection (" + collName
                            + ") with opts " + dbCapOpts);
                }
            }
    }
    ...
    public <T, V> Query<T> find(String kind, Class<T> clazz, String property,
            V value, int offset, int size) {
        return find(kind, clazz, property, value, offset, size, true);
    }

    public <T, V> Query<T> find(String kind, Class<T> clazz, String property,
            V value, int offset, int size, boolean validate) {
    ...
    public <T, V> Query<T> get(Class<T> clazz, Iterable<V> ids) {
        return find(clazz).disableValidation()
    ...
    public <T> List<Key<T>> getKeysByRefs(List<DBRef> refs) {
        ...
        for (String kind : kindMap.keySet()) {
            ...
            List<Key<T>> kindResults = this.<T> find(kind, null)
                    .disableValidation().filter("_id in", objIds).asKeyList();
    ...
    public <T> List<T> getByKeys(Class<T> clazz, Iterable<Key<T>> keys) {
        ...
        for (Key<?> key : keys) {
            ...
            if (kindMap.containsKey(key.getKind()))
                ...
                kindMap.put(
                        key.getKind(),
                        new ArrayList<Key>(Collections.singletonList((Key) key)));
        }
        for (String kind : kindMap.keySet()) {
            List<Object> objIds = new ArrayList<Object>();
            ...
            List kindResults = find(kind, null).disableValidation()
                    .filter("_id in", objIds).asList();
    ...
    public <T, V> T get(Class<T> clazz, V id) {
        return find(getCollection(clazz).getName(), clazz, Mapper.ID_KEY, id,
                0, 1, true).get();
    }
    ...
    public <T> T getByKey(Class<T> clazz, Key<T> key) {
        ...
        if (!kind.equals(keyKind))
            throw new RuntimeException(
                    "collection names don't match for key and class: " + kind
                            + " != " + keyKind);
    ...
    public Key<?> exists(Object entityOrKey) {
        ...
        if (id == null)
            throw new MappingException("Could not get id for "
                    + entityOrKey.getClass().getName());

        String collName = key.getKind();
        if (collName == null)
            collName = getCollection(key.getKindClass()).getName();

        return find(collName, key.getKindClass()).filter(Mapper.ID_KEY,
                key.getId()).getKey();
    }
    ...
    public MongoCollection getCollection(Class clazz) {
        ...
        MongoCollection dbC = getDB().getCollection(collName);
        return dbC;
    }

    public MongoCollection getCollection(Object obj) {
        if (obj == null)
            return null;
        return getCollection(obj.getClass());
    }

    protected MongoCollection getCollection(String kind) {
        if (kind == null)
            return null;
    ...
    private <T> Iterable<Key<T>> insert(MongoCollection dbColl,
        ...
        while (entitiesIT.hasNext()) {
            ...
            savedKeys
                    .add(postSaveGetKey(entity, dbObj, dbColl, involvedObjects));
        }
    ...
    protected DBObject entityToDBObj(Object entity,
    ...
    protected <T> Key<T> postSaveGetKey(T entity, DBObject dbObj,
            MongoCollection dbColl, Map<Object, DBObject> involvedObjects) {
    ...
    protected <T> Key<T> save(MongoCollection dbColl, T entity, WriteConcern wc) {
        ...
        // involvedObjects is used not only as a cache but also as a list of
        // what needs to be called for life-cycle methods at the end.
        LinkedHashMap<Object, DBObject> involvedObjects = new LinkedHashMap<Object, DBObject>();
        DBObject dbObj = entityToDBObj(entity, involvedObjects);
    ...
    protected <T> WriteResult tryVersionedUpdate(MongoCollection dbColl,
        ...
        if (oldVersion != null && oldVersion > 0) {
            ...
            UpdateResults<T> res = update(
                    find(dbColl.getName(), (Class<T>) entity.getClass())
                            .filter(Mapper.ID_KEY, idValue).filter(
                                    versionKeyName, oldVersion), dbObj, false,
                    false, wc);
            ...
            if (res.getUpdatedCount() != 1)
                throw new ConcurrentModificationException("Entity of class "
                        + entity.getClass().getName() + " (id='" + idValue
                        + "',version='" + oldVersion
    ...
    public <T> UpdateResults<T> update(T ent, UpdateOperations<T> ops) {
        ...
        if (mc.getFieldsAnnotatedWith(Version.class).size() > 0) {
            MappedField versionMF = mc.getFieldsAnnotatedWith(Version.class)
                    .get(0);
    ...
    public <T> UpdateResults<T> update(Key<T> key, UpdateOperations<T> ops) {
        ...
        return updateFirst(
                createQuery(clazz).disableValidation().filter(Mapper.ID_KEY,
                        key.getId()), ops);
    }
    ...
    public <T> Key<T> merge(T entity, WriteConcern wc) {
        ...
        if (wr == null) {
            Query<T> query = (Query<T>) createQuery(entity.getClass()).filter(
                    Mapper.ID_KEY, id);
            wr = update(query, new BasicDBObject("$set", dbObj), false, false,
                    wc).getWriteResult();
        }
    ...
    private <T> void postSaveOperations(Object entity, DBObject dbObj,
            Map<Object, DBObject> involvedObjects) {
    ...
    private <T> UpdateResults<T> update(Query<T> query, DBObject u,
        ...
        if (qi.getSortObject() != null && qi.getSortObject().keySet() != null
                && !qi.getSortObject().keySet().isEmpty())
            throw new QueryException("sorting is not allowed for updates.");
        if (qi.getOffset() > 0)
            throw new QueryException(
                    "a query offset is not allowed for updates.");
        if (qi.getLimit() > 0)
            throw new QueryException(
                    "a query limit is not allowed for updates.");
    ...
    public <T> T findAndDelete(Query<T> query) {
        MongoCollection dbColl = ((QueryImpl<T>) query).getCollection();
        // TODO remove this after testing.
        if (dbColl == null)
        ...
        if (log.isTraceEnabled())
            log.trace("Executing findAndModify(" + dbColl.getName()
                    + ") with delete ...");

        DBObject result = dbColl.findAndModify(qi.getQueryObject(),
                qi.getFieldsObject(), qi.getSortObject(), true, null, false,
                false);

        if (result != null) {
            T entity = (T) mapr
                    .fromDBObject(qi.getEntityClass(), result, cache);
            return entity;
        }
    ...
    public <T> T findAndModify(Query<T> query, UpdateOperations<T> ops,
        ...
        if (log.isTraceEnabled())
            log.info("Executing findAndModify(" + dbColl.getName()
                    + ") with update ");
        DBObject res = null;
        try {
            res = dbColl.findAndModify(qi.getQueryObject(),
                    qi.getFieldsObject(), qi.getSortObject(), false,
        ...
        if (res == null)
            ...
            return (T) mapr.fromDBObject(qi.getEntityClass(), res,
                    createCache());
    }
    ...
    public <T> MapreduceResults<T> mapReduce(MapreduceType type, Query q,
        ...
        if (MapreduceType.INLINE.equals(type))
            throw new IllegalArgumentException(
                    "Inline map/reduce is not supported.");
        ...
        MapReduceCommand cmd = new MapReduceCommand(dbColl,
                baseCommand.getMap(), baseCommand.getReduce(),
        ...
        MapreduceResults mrRes = (MapreduceResults) mapr.fromDBObject(
                MapreduceResults.class, mpo.getRaw(), createCache());

        QueryImpl baseQ = null;
        if (!MapreduceType.INLINE.equals(type))
            baseQ = new QueryImpl(outputType, db.getCollection(mrRes
                    .getOutputCollectionName()), this);
    ...
    @SuppressWarnings("rawtypes")
    public <T> MapreduceResults<T> mapReduce(MapreduceType type, Query query,
        ...
        MapReduceCommand cmd = new MapReduceCommand(dbColl, map, reduce,
                outColl, outType, qi.getQueryObject());
    ...
    public static <T> List<DocumentReference> keysAsRefs(List<Key<T>> keys,
            Mapper mapr) {
    ...
    public Durability getWriteConcern(Object clazzOrEntity) {
        ...
        if (clazzOrEntity != null) {
            ...
            if (entityAnn != null && !"".equals(entityAnn.concern()))
                wc = Durability.valueOf(entityAnn.concern());
        }
