...
public class MappedClass {
    ...
    public static List<Class<? extends Annotation>> interestingAnnotations = new ArrayList<Class<? extends Annotation>>(
            Arrays.asList(Embedded.class, Entity.class, Polymorphic.class,
                    EntityListeners.class, Version.class, Converters.class,
                    Indexes.class));
    /** Annotations interesting for life-cycle events */
    private static Class<? extends Annotation>[] lifecycleAnnotations = new Class[] {
            PrePersist.class, PreSave.class, PostPersist.class, PreLoad.class,
            PostLoad.class };
    ...
    protected void basicValidate() {
        ...
        if (!isstatic && clazz.isMemberClass())
            throw new MappingException("Cannot use non-static inner class: "
                    + clazz + ". Please make static.");
    }

    /*
     * Update mappings based on fields/annotations.
     */
    // TODO: Remove this and make these fields dynamic or auto-set some other
    // way
    public void update() {
    ...
    protected void discover() {
        ...
        if (entityLisAnn != null && entityLisAnn.value() != null
                && entityLisAnn.value().length != 0)
            ...
            for (Method m : ReflectionUtils.getDeclaredAndInheritedMethods(cls)) {
                for (Class<? extends Annotation> c : lifecycleAnnotations) {
                    if (m.isAnnotationPresent(c)) {
                        addLifecycleEventMethod(c, m, cls.equals(clazz) ? null
                                : cls);
                    }
                }
            }
        ...
        for (Field field : ReflectionUtils.getDeclaredAndInheritedFields(clazz,
                true)) {
            ...
            if (field.isAnnotationPresent(Transient.class))
                continue;
                    ...
                    && (fieldMods & Modifier.TRANSIENT) == Modifier.TRANSIENT)
                continue;
            else if (mapr.getOptions().actLikeSerializer
                    && ((fieldMods & Modifier.TRANSIENT) == Modifier.TRANSIENT))
                continue;
            else if (mapr.getOptions().ignoreFinals
                    && ((fieldMods & Modifier.FINAL) == Modifier.FINAL))
                continue;
                ...
                update();
                    ...
                    || ReflectionUtils.implementsInterface(field.getType(),
                            Serializable.class)) {
                ...
                if (mapr.getOptions().defaultMapper != null)
                    ...
                    log.warning("Ignoring (will not persist) field: "
    ...
    private void addLifecycleEventMethod(Class<? extends Annotation> lceClazz,
            Method m, Class<?> clazz) {
    ...
    public String toString() {
        return "MappedClass - kind:" + this.getCollectionName() + " for "
                + this.getClazz().getName() + " fields:" + persistenceFields;
    }

    /** Returns fields annotated with the clazz */
    public List<MappedField> getFieldsAnnotatedWith(
    ...
    /**
     * Checks to see if it a Map/Set/List or a property supported by the MangoDB
     * java driver
     */
    public static boolean isSupportedType(Class<?> clazz) {
        ...
        if (clazz.isArray() || Map.class.isAssignableFrom(clazz)
            ...
            if (subType != null && subType != Object.class
    ...
    public DocumentBuilder callLifecycleMethods(
            Class<? extends Annotation> event, Object entity,
            DocumentBuilder dbObject, Mapper mapr) {
        List<ClassMethodPair> methodPairs = getLifecycleMethods((Class<Annotation>) event);
        DocumentBuilder retDbObj = dbObject;
        try {
            Object tempObj = null;
            if (methodPairs != null) {
                HashMap<Class<?>, Object> toCall = new HashMap<Class<?>, Object>(
                ...
                for (ClassMethodPair cm : methodPairs) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug("Calling lifecycle method(@"
                                + event.getSimpleName() + " " + method
                                + ") on " + inst + "");
            ...
            callGlobalInterceptors(event, entity, dbObject, mapr,
                    mapr.getInterceptors());
        }
    ...
    private void callGlobalInterceptors(Class<? extends Annotation> event,
            Object entity, DocumentBuilder dbObj, Mapper mapr,
            Collection<EntityInterceptor> interceptors) {
        for (EntityInterceptor ei : interceptors) {
            if (log.isDebugEnabled())
                log.debug("Calling interceptor method " + event.getSimpleName()
                        + " on " + ei);
    ...
    /**
     * @return the instance if it was found, if more than onw was found, the
     *         last one added
     */
    public Annotation getAnnotation(Class<? extends Annotation> clazz) {
        ArrayList<Annotation> found = foundAnnotations.get(clazz);
        return (found != null && found.size() > 0) ? found
                .get(found.size() - 1) : null;
    }
    ...
    public String getCollectionName() {
        return (entityAn == null || entityAn.value().equals(
                Mapper.IGNORED_FIELDNAME)) ? clazz.getSimpleName() : entityAn
                .value();
    }
