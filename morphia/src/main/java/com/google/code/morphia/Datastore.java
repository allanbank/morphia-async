...
public interface Datastore {
    ...
    <T, V> long delete(Class<T> clazz, V id);

    /** Deletes the given entities (by id) */
    <T, V> long delete(Class<T> clazz, Iterable<V> ids);

    /** Deletes the given entities based on the query */
    <T> long delete(Query<T> q);
     ...
     * @return
     */
    <T> long delete(Query<T> q, Durability wc);

    /** Deletes the given entity (by @Id) */
    <T> long delete(T entity);

    /** Deletes the given entity (by @Id), with the Durability */
    <T> long delete(T entity, Durability wc);
     ...
     * <p>
     * This is the same as:
     * {@code find(clazzOrEntity).filter(property, value); }
     * </p>
     */
     ...
     * <p>
     * This is the same as:
     * {@code find(clazzOrEntity).filter(property, value).offset(offset).limit(size); }
     * </p>
     */
    <T, V> Query<T> find(Class<T> clazz, String property, V value, int offset,
            int size);
     ...
     * Durability
     */
    <T> Iterable<Key<T>> save(Iterable<T> entities, Durability wc);
    ...
    /** Saves the entity (Object) and updates the @Id field, with the Durability */
    <T> Key<T> save(T entity, Durability wc);
     ...
     * Only at the top level of the entity.
     */
    <T> Key<T> merge(T entity, Durability wc);
     ...
     * atomic operation per entity
     */
    <T> UpdateResults<T> updateFirst(Query<T> query, T entity,
            boolean createIfMissing);
     ...
     */
    <T> MapreduceResults<T> mapReduce(MapreduceType type, Query q, String map,
     ...
     */
    @Deprecated
    <T> void ensureIndex(Class<T> clazz, String name, IndexFieldDef[] fields,
     ...
     * (using {@code @Indexed, @Indexes)}
     */
    void ensureIndexes();

    /**
     * Ensures (creating if necessary) the indexes found during class mapping
     * (using {@code @Indexed, @Indexes)}, possibly in the background
     */
    ...
    /** ensure capped MongoCollections for {@code Entity}(s) */
    void ensureCaps();
    ...
    MongoCollection getCollection(Class<?> c);

    Durability getDefaultDurability();

    void setDefaultDurability(Durability wc);
