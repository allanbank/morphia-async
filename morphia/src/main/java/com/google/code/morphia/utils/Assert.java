...
public final class Assert {
    public static <T extends Enum<T>> void enumContains(
            final EnumSet<T> enumSetToLookIn, final T enumToLookFor) {
        Assert.isTrue(enumSetToLookIn.contains(enumToLookFor), "Mode "
                + enumToLookFor + " is not part of the enum given : "
                + enumSetToLookIn);
    }

    public static void implementsSerializable(final Object toTest) {
        if (!Serializable.class.isAssignableFrom(toTest.getClass())) {
            Assert.raiseError("Object of class '" + toTest.getClass()
    ...
    public static void isSame(final Object a, final Object b) {
        Assert.isTrue(a == b,
                "Expected to be the same (not only equal, but same reference)");
    }
    ...
    public static void parameterArraysOfSameLength(final String argNames,
        ...
        parameterLegal(length >= 2,
                "Parameter objects was expected to contains at least two arrays.");
        ...
        for (int i = 0; i < length; i++) {
            ...
            if ((i > 0) && (sizeOfO != size)) {
                raiseError("At least one of '" + argNames
                        + "' differs in length");
            }
    ...
    public static <T> void parameterInCollection(final T object,
            final Collection<? super T> collection) {
        if (collection == null) {
            Assert.raiseError("'collection' is null");
        }
        if (!collection.contains(object)) {
            Assert.raiseError("Parameter '" + object + "' not in collection '"
                    + collection + "'");
        }
    }
    ...
    public static void parameterInRange(final long value, final long min,
            final long max, final String name) {
        Assert.isTrue((min <= value) && (value <= max), "Parameter '" + name
                + "' must be in range of " + min + " <= " + name + " <= " + max
                + ". Current value was " + value);
    }

    public static void parameterInstanceOf(final Object obj,
            final Class<?> class1, final String paramName) {
        if (!class1.isAssignableFrom(obj.getClass())) {
            Assert.raiseError("Parameter '" + paramName + "' from type '"
                    + obj.getClass().getName()
                    + "' was expected to implement '" + class1 + "'");
        }
    }

    public static void parameterIterableDoesNotContainNull(final String string,
            final Iterable<?> collection) {
        Assert.parametersNotNull("string, collection", string, collection);
        for (final Object object : collection) {
            if (object == null) {
                raiseError("Iterable Parameter '" + string
    ...
    public static void parameterNotEmpty(final Collection obj,
    ...
    public static void parameterNotEmpty(final Iterable obj,
            final String paramName) {
    ...
    public static void parametersNotNull(final String nameOfParameters,
            Object... objects) {
        ...
        for (final Object object : objects) {
            if (object == null) {
                Assert.raiseError(msgPrefix + "'" + nameOfParameters + "' "
                        + msgSuffix);
            }
        }
    }
